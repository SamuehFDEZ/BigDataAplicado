...
val topThreePerMaritalDF = bankDF
  .withColumn("ranking", row_number().over(Window.partitionBy("marital").orderBy(desc("balance"))))
...
val costOfVehiclesDF = vehiclesDF
  // We are not interested in rows without a valid_cost_in_credits
  .filter(col("cost_in_credits") =!= "NA")
  // Also let's ensure cost_in_credits is a number so we can do arithmetics with it
  .withColumn("cost_in_credits", col("cost_in_credits").cast(IntegerType))
  // For each row, let's add a column with the minimum cost for that particular vehicle class
  .withColumn("min_cost", min(col("cost_in_credits")).over(Window.partitionBy("vehicle_class").orderBy("cost_in_credits")))
  // Now, the cost difference is the cost in credits minus the minimum one
  .withColumn("cost_difference", col("cost_in_credits") - col("min_cost"))
  // Let's get all the columns, but in the order we want!
  .select({
    val reorderedCols = Seq("vehicle_class", "cost_in_credits", "min_cost", "cost_difference")    
    // Note: df.columns provides a sequence of strings with the names of the columns
    (reorderedCols ++ vehiclesDF.columns.diff(reorderedCols)).map(col)
  // Note2: the scala syntax ls:_* converts a iterable into a series of var args
  }:_*)
  // Let's order by vehicle class and cost
  .orderBy(col("vehicle_class"), desc("cost_in_credits"))
...
val resultDF = charactersDF.select("homeworld", "name")
  .join(planetsDF.select("name", "gravity"), 
    charactersDF("homeworld") === planetsDF("name"))
  .select(charactersDF("name"), planetsDF("name"), col("gravity"))
