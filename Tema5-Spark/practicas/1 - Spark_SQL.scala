...
val nullCount = vehiclesDF.filter(
  $"cargo_capacity".isNull || 
  $"cargo_capacity" === "" || 
  $"cargo_capacity" === "NA" || 
  $"cargo_capacity" === "None" || 
  $"cargo_capacity".isNaN
).count()

// COMMAND ----------

val vehiclesFilteredDF = vehiclesDF
  .filter(!$"cargo_capacity".isin("NA", "none"))
  .withColumn("cargo_capacity", $"cargo_capacity".cast("long"))
  .filter($"cargo_capacity" > 70)
...
def removeInvalidValues(df: DataFrame, column: String): DataFrame = {
  df.filter(
    col(column).isNotNull &&                // Excluye nulos
    !col(column).isin("NA", "None", "") &&  // Excluye "NA", "None" y vacíos
    !col(column).isNaN                      // Excluye NaN (solo numéricos)
  )
}
...
val cleanedVehiclesDF = removeInvalidValues(vehiclesDF, "passengers")

val vehiclesAvgDF = cleanedVehiclesDF
  .groupBy("vehicle_class")
  .agg(avg("passengers").as("passengers_avg"))
  .orderBy(desc("passengers_avg"))
...
val charactersEyeDF = charactersDF
  .groupBy("eye_color")
  .agg(count("eye_color").as("most_frequent_color"))
  .orderBy(desc("most_frequent_color"))
  .limit(1)
...
// MAGIC %sql
// MAGIC select gender, count(*) as count 
...
val nullCount = netflixDF.filter(
  $"type".isNull
).count()
...
val getYearAdded = udf((date: String) => date.takeRight(4))

val netflixMoviesDF = netflixDF
  .filter(col("date_added").isNotNull)
  .select(col("title"), getYearAdded(col("date_added")).alias("release_year"))
  .distinct()
