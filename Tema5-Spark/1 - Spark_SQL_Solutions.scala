...
val vehiclesFilteredDF = vehiclesDF
  .filter(!col("cargo_capacity").isin("NA", "none"))
  .withColumn("cargo_capacity", col("cargo_capacity").cast("long"))
...
// MAGIC ---
// MAGIC From Vehicles dataframe loaded in previous exerciese, get the average of passengers  by vehicle class
// MAGIC
...
val vehiclesAvgDF = vehiclesDF
  ...
  .agg(avg(col("passengers")).alias("avg_passengers")) 
...
val netflixMoviesDF = netflixDF.filter(col("type") === "Movie")

display(netflixMoviesDF)

// COMMAND ----------

val getYearAdded = udf((s: String) => s.takeRight(4))
